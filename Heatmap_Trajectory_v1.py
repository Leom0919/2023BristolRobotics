...
base_dir = "E:\\Dissertation\\Completedlc\\July3_3"  # 设置基础目录
...
for idx, filename in enumerate(filenames):
    ...
    # 使用二值化的方法确定哪些像素被触摸
    binary_heatmap = (heatmap > 0).astype(int)
    ...
    # 为Heatmap添加标题和触摸面积
    plt.title(f"Touch heatmap for {texture_dict[idx + 1]}")
    # touch_area = np.sum(heatmap)  # 使用heatmap的值的总和计算触摸面积
    # touch_area = round(np.sum(heatmap), 1)  # 使用heatmap的值的总和计算触摸面积
    plt.figtext(0.5, 0.02, f"Touch Area: {touch_area} pixels",
    ...
    trajectory_length = round(compute_trajectory_length(x_coords, y_coords), 1)
    plt.figtext(0.5, 0.02, f"Trajectory Length: {trajectory_length} units",
                horizontalalignment='center', fontsize=9)
